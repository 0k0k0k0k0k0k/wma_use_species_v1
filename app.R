...
save_report_map_png <- function(poly,
  ...
  if (identical(map_type, "points")) {
    ...
    if (!is.null(heat_data) && nrow(heat_data) > 0) {
      ...
      if (nrow(df) > 0) {
        if ("heat_intensity" %in% names(df)) {
          intensity <- df$heat_intensity
        } else {
          intensity <- rep(1, nrow(df))
        }
        
        intensity[is.na(intensity)] <- 1
        
        max_intensity <- suppressWarnings(max(intensity, na.rm = TRUE))
        ...
        idx <- ceiling((intensity / max_intensity) * (ncols - 1)) + 1
        idx[idx < 1] <- 1
        idx[idx > ncols] <- ncols
...
server <- function(input, output, session) {
  ...
  output$annual_plot <- renderPlot(
    ...
    width = function() {
      w <- session$clientData$output_annual_plot_width
      if (is.null(w) || length(w) == 0 || is.na(w) || w < 100) 800 else w
    },
    height = function() {
      250
    },
