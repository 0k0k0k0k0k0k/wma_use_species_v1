...
server <- function(input, output, session) {
  ...
  output$download_report <- downloadHandler(
    ...
    content = function(file) {
      ...
      tmp_dir <- file.path(tempdir(), paste0("report_", as.integer(Sys.time())))
      ...
      ok_copy <- file.copy(report_template_file, report_rmd, overwrite = TRUE)
      if (!ok_copy) {
        stop("Could not copy report template into temp directory.")
      }
      
      site_name_val <- current_site_name()
      selected_year_val <- input$selected_year
      selected_species_val <- if (is.null(input$selected_species) || input$selected_species == "__none__") "" else input$selected_species
      ...
      report_heat_metric <- if (is.null(input$heat_metric)) "" else input$heat_metric
      ...
      rendered_file <- rmarkdown::render(
        ...
        params = list(
          ...
          annual_plot_path = normalizePath(annual_png, winslash = "/", mustWork = TRUE),
          map_path = normalizePath(map_png, winslash = "/", mustWork = TRUE),
          report_date = report_date_val,
          heat_metric = report_heat_metric
        ),
      ...
      ok_final <- file.copy(rendered_file, file, overwrite = TRUE)
      if (!ok_final) {
        stop("Could not copy rendered PDF to download location.")
      }
    }
  )
